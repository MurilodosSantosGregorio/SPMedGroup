...
const URL = 'http://localhost:5000/api/Medicos';
class AtualizarConsultas extends Component {
    ...
    carregarStatusConsulta() {
        fetch("http://localhost:5000/api/StatusConsulta",
            {
                method: 'GET',
                headers: {
                    "Content-Type": "application/json",
        ...
        return (
            <div>
                ...
                <form className="grid-form" onSubmit={this.atualizarConsulta.bind(this)}>
                    <div><select value={this.state.CrmMedico} onChange={this.atualizaCrmMedico.bind(this)}>
                        <option  >Selecione o Médico</option>
                        {
                            this.state.listaMedicos.map((element) => {
                                return <option key={element.crm} value={element.crm}>{element.idUsuarioNavigation}</option>
                            })
                        }
                    </select></div>
                    ...
                    <div><input className="consultaAtualizar"  type="number" placeholder={"Id da consulta:   " + parseInt()} onChange={() => alert('Não tem permissão para alterar')} /></div>
